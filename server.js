...
app.delete("/api/delorder/:uid",async(req,res)=>
{
    var result = await OrderModel.deleteOne({_id:req.params.uid})//{ acknowledged: true, deletedCount: 1 }
    if(result.deletedCount===1)
    {
        res.status(200).send({statuscode:1})
    }
...
{
    var result = await CartModel.deleteOne({_id:req.params.uid})//{ acknowledged: true, deletedCount: 1 }
    if(result.deletedCount===1)
    {
        res.status(200).send({statuscode:1})
    }
...
app.delete("/api/delprod/:uid",async(req,res)=>
{
    var result = await ProdModel.deleteOne({_id:req.params.uid})//{ acknowledged: true, deletedCount: 1 }
    if(result.deletedCount===1)
    {
        res.status(200).send({statuscode:1})
    }
...
{
    var result = await CatModel.deleteOne({_id:req.params.uid})//{ acknowledged: true, deletedCount: 1 }
    if(result.deletedCount===1)
    {
        res.status(200).send({statuscode:1})
    }
...
app.put("/api/updatecategory",upload.single('catpic'),async(req,res)=>
{
    try
    {
        ...
        else
        {
            ...
            if(req.body.oldpicname!=="default_image.jpg")
            {
...
app.put("/api/updateproduct",upload.single('catpic'),async(req,res)=>
{
    try {
        ...
        const updateresult = await ProdModel.updateOne({ _id: req.body.id },{$set: {pname: req.body.pname,Rate: req.body.rate,Discount: req.body.dis,Stock: req.body.stock,Description: req.body.descp,picture: picturename,addedon: new Date()}});
...
app.post('/api/contact', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ msg: 'Error saving message, please try again.' });
    }
});
